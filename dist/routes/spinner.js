...
router.delete('/sessions', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const deletedSessions = await GameSession_1.default.find({
            userId: req.user._id,
        ...
        await GameSession_1.default.deleteMany({
            userId: req.user._id,
            cardNumber: { $in: selectedNumbers },
            betAmount: betAmount,
            gameType: 'spinner'
...
router.post('/history', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const spinnerHistory = new SpinnerHistory_1.default({
        ...
        const winner = await User_1.default.findById(req.user._id);
        if (winner) {
...
router.get('/history', async (req, res) => {
    try {
        const history = await SpinnerHistory_1.default.find()
...
router.get('/history/user/:userId', async (req, res) => {
    try {
        const history = await SpinnerHistory_1.default.find({ winnerId: req.params.userId })
            .populate('winnerId', 'phone')
            .sort({ createdAt: -1 });
        res.json(history);
    }
...
router.get('/spinner/history/bet/:amount', async (req, res) => {
    try {
        const amount = parseInt(req.params.amount);
        const history = await SpinnerHistory_1.default.find({ betAmount: amount })
            .populate('winnerId', 'phone')
            .sort({ createdAt: -1 });
        res.json(history);
    }
